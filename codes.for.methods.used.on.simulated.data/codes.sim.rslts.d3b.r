...
sim.normal.dat  <- read.table(".../sim.data.d3b.normal.csv", header = TRUE, sep = ",")
sim.cauchy.dat  <- read.table(".../sim.data.d3b.cauchy.csv", header = TRUE, sep = ",")
...
d3b.normal.dat  <- read.table(".../normal.d3b.corr.csv", header = FALSE, sep = ",")
d3b.cauchy.dat  <- read.table(".../cauchy.d3b.corr.csv", header = FALSE, sep = ",")


dnm <- dcc.normal.mat <- d3b.normal.dat[,(2*c(0:999)) + 1]
dcm <- dcc.cauchy.mat <- d3b.cauchy.dat[,(2*c(0:999)) + 1]
...
rho.vec1 <- array(,T)
for(t in 1:T){ rho.vec1[t] <- rho <- dnorm(t, 250, (15*4)) }

rho.vec <- (0.80/max(rho.vec1))*rho.vec1
...
lines(d3b.cauchy.dat[,3], col = "red")
lines(rho.vec, lwd = 2)
