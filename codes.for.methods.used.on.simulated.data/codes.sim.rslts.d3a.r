...
sim.normal.dat  <- read.table(".../sim.data.d3a.normal.csv", header = TRUE, sep = ",")
sim.cauchy.dat  <- read.table(".../sim.data.d3a.cauchy.csv", header = TRUE, sep = ",")
...
d3a.normal.dat  <- read.table(".../normal.d3a.corr.csv", header = FALSE, sep = ",")
d3a.cauchy.dat  <- read.table(".../cauchy.d3a.corr.csv", header = FALSE, sep = ",")


dnm <- dcc.normal.mat <- d3a.normal.dat[,(2*c(0:999)) + 1]
dcm <- dcc.cauchy.mat <- d3a.cauchy.dat[,(2*c(0:999)) + 1]
...
rho.vec1 <- array(,T)
for(t in 1:T){ rho.vec1[t] <- rho <- dnorm(t, 250, (15*3)) }
